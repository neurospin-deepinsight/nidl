...
class KernelMetric(BaseEstimator):
    """
    This class provides an interface for fast kernel computation. It computes
    the similarity matrix between input samples based on Kernel Density
    Estimation (KDE) weighting scheme [1, 2].
    Concretely, it computes the following similarity matrix between x1, ...,
    xn:
    .. math::
    ...
    def set_bandwidth(self, X):
        ...
        if isinstance(self.bandwidth, (float, int)):
            self.sqr_bandwidth_ = np.sqrt(
                np.diag([self.bandwidth for _ in range(X.shape[1])])
            )
        ...
        elif isinstance(self.bandwidth, (list, np.ndarray)):
            if len(self.bandwidth) != X.shape[1]:
                raise ValueError(
                    "Length of `bandwidth` must match number of features in X."
                )
