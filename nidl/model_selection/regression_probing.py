...
class RegressionProbingCV(MultiTaskProbingCV):
    """Probe the representation of an embedding estimator on a regression
    task.

    RegressionProbingCV aims at evaluating the representation of an
    embedding model on a regression dataset. Input data are embedded
    using a fitted embedding estimator and this representation is evaluated on
    the regression task using a scikit-learn compatible regressor. A
    cross-validation scheme is used for evaluating the regressor on data
    seen during `fit`.

    RegressionProbingCV implements `fit`, `predict` and `score` to
    respectively fit the regressor on the task, predict the labels on new
    data and score the regressor on new data.
    ...
    regressor: sklearn.base.BaseEstimator or None, default=None
        The regression probe used to evaluate the data embedding on the
        regression task. If None (default), a linear regression is used.
    ...
    scoring: str, callable, list of str, dict or None, default=None
        ...
        - None, the default probe's evaluation criteria (R^2).
    ...
    n_jobs: int or None, default=None
        Number of jobs to run in parallel.  Training the regressor and
    ...
    ``cv_results_``: dict
        ...
        regression task. The structure is as follows:

        {
            "test_score": list of float
                The evaluation score of the regressor on each cross-validation
                test split. Suffix `_score` in `test_score` changes to
                specific metric name like `test_r2` when multiple scoring
            ...
            "estimator": sklearn.base.BaseEstimator
                Regressor object fitted on each CV training split.
        }

    ``regressor_``: sklearn.base.BaseEstimator
        Regressor fitted on the training set.

    ``scorer_``: callable
        Score function used to score the regressor.
    ...
    def fit(
        ...
        """Fit the regressor on the training data embedding.
        ...
        self.scorer_ = check_scoring(
            estimator=self.regressor, scoring=self.regression_scoring
        )
        ...
        # Refit the regressor on the entire dataset
        self.regressor_ = clone(self.regressor).fit(X_task, y_task)
        self.fitted_ = True
        return self
    ...
    def score(self, test_dataloader: data.DataLoader):
        ...
        return self.scorer_(self.regressor_, X, y)
