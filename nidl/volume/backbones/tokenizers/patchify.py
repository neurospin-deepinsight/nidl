...
class PatchEmbed3D(nn.Module):
    """Converts 3D volume to a sequence of small volume embeddings.
    Optionally, it adds positional embeddings to the sequence of volume
    embeddings. This is useful for ViT-based models (e.g. DINO, DINOv2, I-JEPA)
    ...
    patch_size: int or tuple of (int, int, int)
        3d patch size over the volume size.
    ...
    learnable_pos_emb: bool, default=False
        If True, learns positional embeddings when using sin-cos
        initialization. It is automatically learned if sin-cos
        initialization is disabled.

    image_size: None, int or (int, int, int), default=128
    ...
    def __init__(
        ...
        if self.add_pos_emb:
            # Fixed-size positional embeddings. Can be interpolated to
            ...
            if self.sincos_pos_emb:
                ...
                self.pos_emb = nn.Parameter(
                    torch.zeros(
                        1, self.embed_dim, self.N_H, self.N_W, self.N_D
                    )
                )
                trunc_normal_(self.pos_emb, std=0.02)

        # Volume -> patch embeddings projection
        self.proj = nn.Conv3d(
            in_channels=self.num_channels,
            out_channels=self.embed_dim,
            kernel_size=(self.P_H, self.P_W, self.P_D),
            stride=(self.P_H, self.P_W, self.P_D),
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        _, _, H, W, D = x.shape
        assert (
            (H % self.P_H == 0) and (W % self.P_W == 0) and (D % self.P_D == 0)
        ), (
            f"Image sizes {H}x{W}x{D} must be divisible by patch sizes "
            f"{self.P_H}x{self.P_W}x{self.P_D}"
        )
        N_H, N_W, N_D = (
            H // self.P_H,
            W // self.P_W,
            D // self.P_D,
        )  # Number of patches in height, width and depth
