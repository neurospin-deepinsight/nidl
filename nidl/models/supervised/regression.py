...
class DeepRegressor(BaseEstimator):
    """Supervised deep model for regression tasks.
    ...
    def __init__(self, 
        ...
        encoder_kwargs: dictionary or None, default=None
            ...
            By default, it sets the input channels to 1 and the output dimension to 1 (univariate regression).
             Examples: 
                * encoder='alexnet_3d', encoder_kwargs={"in_channels": 1, "n_embedding": n_targets}
                    builds an AlexNet with 1 input channel (default) and output dimension 'n_targets'.
                * encoder='linear', encoder_kwargs={"in_features": 10, "out_features": n_targets, "bias": True}
                    builds a linear layer (with intercept) with input dimension 10, output dimension 'n_targets'.
                * encoder='mlp', encoder_kwargs={"layers": [10, 4, 3, 2], "n_embedding": n_targets} 
                    builds an MLP with 4 hidden layers, the input dimension being 10. 
                * encoder='cebra', encoder_kwargs={"num_input": 10, "n_embedding": n_targets} 
                    builds an MLP with input dimension 10 (adapted from CEBRA)

        loss: str in {"l1", "l2", "mse", "mae"} or nn.Module, default="l1"
            Loss to optimize. If a string is provided, it will be mapped to the corresponding
            PyTorch loss function. By default, it uses the L1 loss (mean absolute error).
            Note: 'l1' and 'mae' are equivalent, as well as 'l2' and 'mse'.
        ...
        **kwargs: dict, optional
        ...
        loss_: nn.Module
            The loss function used for training the model (default: CrossEntropyLoss).
        """
    ...
    def _build_loss(self, loss: Union[str, nn.Module]) -> nn.Module:
        ...
        Parameters
        ----------
        loss: str in {'l1', 'l2', 'mae', 'mse'} or nn.Module
            Which loss function to use for regression.
        Returns
        -------
        loss_fn: nn.Module
