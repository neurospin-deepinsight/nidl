...
class OpenBHB(Dataset):
    ...
    REVISION = "4806b5b36bd0982452c871f240cfc26e6934690c"  # commit ID
    ...
    def download_dataset_split(
        ...
        """
        img_regex = {
            "vbm_roi": "sub-*cat12vbm_desc-gm_ROI.npy",
            "vbm": "sub-*cat12vbm_desc-gm_T1w.npy",
            "quasiraw": "sub-*quasiraw_T1w.npy",
            "fs_xhemi": "sub-*xhemi_T1w.npy",
            "fs_desikan_roi": "sub-*desikan_ROI.npy",
            "fs_destrieux_roi": "sub-*destrieux_ROI.npy",
        }
        split = "train" if split == "train" else "val"
        allow_patterns = [
            f"{split}/derivatives/sub-*/ses-*/{img_regex[m]}" for m in modality
        ]
        ignore_patterns = []
        len_all = 0
        if incremental:
    ...
    def make_dataset(self, split: str):
        ...
        samples: list of tuple
            ...
            - path(s) is either a single file path (str) or a tuple of file
        ...
        img_paths = {
            ...
            "fs_destrieux_roi": "sub-%i_preproc-freesurfer_desc-destrieux_ROI"
            ".npy",
        }
        ...
        for entry in data:
            ...
            if len(sample) == 1:  # one modality
                samples.append((sample[0], target))
            else:  # multi-modal openBHB
                samples.append((tuple(sample), target))
        return samples
    ...
    def _load_sample(self, sample_path):
        if isinstance(sample_path, str):
            ...
            return np.load(sample_path)[0].astype(np.float32)
        else:
            # Returns a dict {<modality>: array}
            for s in sample_path:
                self.get_or_download_file(s)
            return {
                mod: np.load(s)[0].astype(np.float32)
